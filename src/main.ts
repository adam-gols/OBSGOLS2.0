...
async function initializeWidget(): Promise<void> {
  ...
  logger.info('GOLS Widget starting up', {
    module: 'MAIN',
    action: 'STARTUP',
    data: {
      isOBS,
  ...
  try {
    ...
    logger.info('GOLS Widget initialization successful', {
      module: 'MAIN',
    ...
    logger.critical('Failed to initialize GOLS Widget', {
      module: 'MAIN',
...
if (import.meta.env.DEV) {
  ...
  logger.debug('Debug objects attached to window.GOLS_DEBUG', {
    module: 'MAIN',
    action: 'DEBUG_SETUP'
  });
}
